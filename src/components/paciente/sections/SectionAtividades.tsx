...
import {
  Award, Trophy, Zap, CheckCircle2, Clock, TrendingUp, Activity, Star, ChevronRight, Target
} from "lucide-react";
...
export default function SectionAtividades() {
  ...
  return (
    <div className="lg:max-w-5xl mx-auto space-y-8">
      <div>
      ...
      <Card className="border-none shadow-sm bg-slate-900 text-white overflow-hidden relative">
        <div className="absolute top-0 right-0 p-8 opacity-10 rotate-12 transform scale-150"><Trophy className="h-24 w-24" /></div>
        <CardContent className="p-8 relative z-10">
          <div className="flex flex-col md:flex-row items-center justify-between gap-8">
            <div className="space-y-4 text-center md:text-left">
              <div className="space-y-1">
                <Badge className="bg-indigo-500 text-white hover:bg-indigo-500 border-none font-black text-[10px] uppercase tracking-widest px-3 h-6">Nível Iniciante</Badge>
                <h2 className="text-3xl font-black italic tracking-tighter">Em busca do próximo nível!</h2>
              </div>
              <div className="max-w-md space-y-2">
                <div className="flex justify-between text-xs font-bold text-slate-400"><span>0 XP</span><span>1000 XP para elevar a barra</span></div>
                <Progress value={0} className="h-3 bg-white/10" />
              </div>
            </div>
            <div className="flex gap-4">
              <div className="text-center">
                <div className="h-16 w-16 bg-white/10 rounded-2xl flex items-center justify-center mb-2 border border-white/10 shadow-lg"><Zap className="h-8 w-8 text-amber-400" /></div>
              ...
              <div className="text-center">
                <div className="h-16 w-16 bg-white/10 rounded-2xl flex items-center justify-center mb-2 border border-white/10 shadow-lg"><Trophy className="h-8 w-8 text-indigo-400" /></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="grid gap-4">
            {activities.map((activity) => (
              <Card key={activity.id} className={cn("border-none shadow-sm transition-all overflow-hidden group items-center", activity.completed ? "bg-white/60 opacity-80" : "bg-white hover:translate-x-1")}>
                <CardContent className="p-0">
                  <div className="flex items-stretch">
                    <div className={cn("w-16 flex items-center justify-center shrink-0", activity.completed ? "bg-emerald-50 text-emerald-600" : "bg-indigo-50 text-indigo-600")}>
                      <activity.icon className="h-6 w-6" />
                    </div>
                    <div className="p-5 flex-1 flex items-center justify-between gap-4">
                      <div className="min-w-0">
                        <Badge variant="outline" className="text-[9px] font-black uppercase tracking-tighter mb-1 h-4 border-slate-200 text-slate-400">{activity.category}</Badge>
                        <h4 className={cn("font-black text-slate-900 leading-tight truncate", activity.completed && "line-through decoration-emerald-500/50")}>{activity.title}</h4>
                        <p className="text-[10px] font-bold text-slate-400 mt-0.5">{activity.time} • +{activity.points} pts</p>
                      </div>
                      <Button size="sm" variant={activity.completed ? "secondary" : "default"} className={cn("rounded-xl font-black text-xs px-5 shadow-sm", activity.completed ? "bg-emerald-100 text-emerald-700 hover:bg-emerald-100 cursor-default" : "bg-slate-900 text-white hover:bg-slate-800")}>
          ...
          <div className="bg-indigo-50/50 rounded-2xl p-6 border-2 border-dashed border-indigo-100/50 flex flex-col items-center text-center gap-2">
            <div className="h-10 w-10 bg-indigo-100 rounded-full flex items-center justify-center mb-2"><TrendingUp className="h-6 w-6 text-indigo-600" /></div>
        ...
        <div className="space-y-8">
          <section className="space-y-4">
            <div className="flex items-center gap-2"><Award className="h-5 w-5 text-indigo-600" /><h2 className="text-lg font-black text-slate-900 leading-none">Conquistas</h2></div>
