...
export default function VoiceAssessment({ serviceType, pacienteId, patientName, patientAge, patientSex, onAssessmentComplete, appendMode, onAppendCapture, mode = 'voice', contextPrefix, onPainExtracted, painRegionsCatalog, painMap, myidContext, perfilProfissional, initialRecord }: VoiceAssessmentProps) {
  ...
  const processAssessment = async () => {
    ...
    try {
      ...
      let converteu = false;
      // Gravação/arquivo LONGO: o WAV 16kHz fica enorme (~2 MB/min → 24 min ≈
      ...
      // (~10x menor) e NÃO convertemos para WAV. Considera longo por TEMPO
      // (>10 min gravados) OU por TAMANHO do arquivo comprimido (>8 MB) — este
      // último cobre o "Enviar áudio" (upload), onde recordingTime é 0.
      const audioLongo = !!audioBlob && (recordingTime > 600 || audioBlob.size > 8 * 1024 * 1024);
