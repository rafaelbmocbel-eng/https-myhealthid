...
export default function AtualizarTratamentoBanner({ pacienteId, onDone }: { pacienteId: string; onDone?: () => void }) {
      ...
      <div className="rounded-xl border border-primary/30 bg-primary/5 p-3 flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-[11px] text-muted-foreground leading-snug">
            Atualize o tratamento ({cfg!.nome}) com o que você acrescentou — você revisa antes de aplicar.
          </p>
        </div>
        <div className="flex items-center gap-1.5 shrink-0">
          <Button size="sm" className="h-8 gap-1.5 text-xs" onClick={() => atualizar.mutate()} disabled={atualizar.isPending}>
            {atualizar.isPending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <RefreshCw className="h-3.5 w-3.5" />}
