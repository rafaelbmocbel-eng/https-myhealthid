...
export function gerarProtocoloAutomatico(
  ...
  // Return ALL exercises for selected areas (no slicing)
  function getExercicios(areas: string[]): ExercicioSugerido[] {
    const todos: ExercicioSugerido[] = [];
    const nomesSeen = new Set<string>();
    areas.forEach(area => {
      const exs = EXERCICIOS_POR_DEMANDA[area] || [];
      exs.forEach(ex => {
        if (!nomesSeen.has(ex.nome)) {
          nomesSeen.add(ex.nome);
          todos.push(ex);
        }
      });
    });
  ...
  // Return ALL techniques for selected areas (no slicing)
  function getTecnicas(areas: string[]): TecnicaSugerida[] {
    ...
    areas.forEach(area => {
      ...
      techs.forEach(t => {
        if (!nomesSeen.has(t.nome)) {
          nomesSeen.add(t.nome);
          todas.push(t);
        }
      });
    });
  ...
  const fases: FaseProtocolo[] = [
    {
      ...
      exercicios: getExercicios(temRegulacao ? ['Regulação'] : areasCriticas),
    ...
    },
    {
      ...
      exercicios: getExercicios(temPsico ? ['Psico-comportamental', 'Dor Percebida'] : ['Estrutural', 'Funcionalidade']),
    ...
    },
    {
      ...
      exercicios: getExercicios(['Estrutural', 'Funcionalidade']),
    ...
    },
    {
      ...
      exercicios: getExercicios([...areasAltas, 'Funcionalidade']),
      tecnicas: [
