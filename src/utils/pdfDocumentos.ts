...
export async function gerarLaudoCinetico(input: BaseInput & { dados: DocLaudoCineticoData & { corpo?: string } }): Promise<jsPDF> {
  const doc = new jsPDF();
  await drawHeader(doc, input.clinica);
  let y = drawTitle(doc, 'Laudo Cinético-Funcional', 50);

  // Texto organizado (mesmo do editor/prévia). Se o usuário editou à mão, usa o
  // texto dele; senão monta o padrão a partir dos campos.
  const text = input.dados.corpo?.trim()
    ? input.dados.corpo
    : corpoPadraoDocumento('laudo_cinetico', input, input.dados) || '';

  y = drawBody(doc, text, y + 4);
  // Assinatura flui logo após o conteúdo (evita página 2 quase vazia).
