...
export default function ProfessionalHub() {
  ...
  const handleInvite = async (paciente?: any) => {
    const registerUrl = `${window.location.origin}/paciente/cadastro?professional_id=${user?.id}`;
    if (paciente) {
  ...
  const filtered = pacientesRes.filter((p: any) =>
    `${p.nome} ${p.sobrenome || ''} ${p.email || ''} ${p.telefone || ''}`.toLowerCase().includes(searchTerm.toLowerCase())
  );
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            <h1 className="text-2xl font-black text-slate-900 flex items-center gap-2">
              <Award className="h-7 w-7 text-indigo-600" /> Gestão de Engajamento MyID
            </h1>
            <p className="text-muted-foreground text-sm">Monitore o engajamento, pontos e hábitos dos seus pacientes.</p>
          </div>
          ...
          <Card className="border-none shadow-sm bg-indigo-50 border-indigo-100">
            <CardContent className="p-4 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xl font-black text-indigo-900">{pacientesRes.length}</p>
              </div>
            </CardContent>
          </Card>
                ...
                >
                  ...
                  <div className="flex items-center gap-4 md:w-36 justify-around">
                    <div className="flex flex-col items-center">
                      <BookOpen className={cn("h-4 w-4 mb-1", "text-emerald-500")} />
                  ...
                  <div className="shrink-0 flex items-center gap-1">
                    <Button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        copyPortalLink(paciente);
                      }}
                      title="Copiar Link do Portal"
                    >
                      {copiedId === paciente.id ? <Check className="h-4 w-4 text-emerald-500" /> : <Link2 className="h-4 w-4" />}
                    </Button>
                    <Button
                      variant="ghost"
                      size="icon"
