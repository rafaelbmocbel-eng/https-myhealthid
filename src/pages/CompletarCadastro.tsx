...
export default function CompletarCadastro() {
  ...
  useEffect(() => {
    if (!token) return;
    (async () => {
      const { data, error } = await (supabase as any).rpc('get_paciente_completar_publico', { p_token: token });
      if (error || !data || data.length === 0) {
        setErro('Link inválido ou expirado. Solicite um novo ao seu profissional.');
      } else {
        const p = data[0] as PacienteBasico;
        setPaciente(p);
        if (p.cadastro_status === 'completo') {
          setErro('Seu cadastro já foi concluído. Acesse seu portal normalmente.');
        } else {
          setForm(f => ({
            ...f,
            nome: p.nome || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await (supabase as any).rpc('salvar_cadastro_paciente_publico', {
        p_token: token, p_data: payload,
      });
      ...
      else if (msg.includes('token_invalido')) friendly = 'Link inválido ou expirado.';
  ...
  if (erro) {
    return (
      <div className="min-h-[100dvh] flex flex-col items-center justify-center bg-background p-6 text-center gap-3">
        <ShieldCheck className="h-12 w-12 text-muted-foreground/40" />
        <h1 className="text-lg font-bold text-foreground">Não foi possível abrir o cadastro</h1>
    ...
    <div className="min-h-[100dvh] bg-background pb-12">
      <header className="px-4 py-5 border-b border-border/40 bg-card sticky top-0 z-10">
        <div className="max-w-xl mx-auto flex items-center gap-3">
          <LogoIcon size={36} />
          <div>
            <div className="text-[11px] text-muted-foreground">Cadastro do paciente</div>
            <div className="text-sm font-bold text-foreground leading-tight">
              {paciente?.terapeuta_nome}
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-xl mx-auto p-4 sm:p-6">
        <div className="mb-5">
          <h1 className="text-lg sm:text-xl font-bold text-foreground">Olá, {paciente?.nome}!</h1>
          <p className="text-xs text-muted-foreground mt-1">
            Complete seu cadastro para que possamos oferecer um atendimento mais seguro e personalizado.
