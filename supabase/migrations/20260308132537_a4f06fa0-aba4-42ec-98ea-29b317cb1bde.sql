...
ALTER TABLE public.funil_leads ADD COLUMN IF NOT EXISTS forma_pagamento TEXT;
ALTER TABLE public.funil_leads ADD COLUMN IF NOT EXISTS agendamento_id UUID REFERENCES public.agendamentos(id);
ALTER TABLE public.funil_leads ADD COLUMN IF NOT EXISTS dados_conversa JSONB DEFAULT '[]'::jsonb;
ALTER TABLE public.funil_leads ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now();
...
DROP POLICY IF EXISTS "Terapeutas gerenciam seu funil" ON public.funil_config;
...
DROP POLICY IF EXISTS "Terapeutas gerenciam leads" ON public.funil_leads;
CREATE POLICY "Terapeutas gerenciam leads" ON public.funil_leads FOR ALL TO authenticated
  USING (auth.uid() = terapeuta_id) WITH CHECK (auth.uid() = terapeuta_id);

-- Público insere leads (via funil)
DROP POLICY IF EXISTS "Público insere leads" ON public.funil_leads;
...
-- Indexes
CREATE INDEX IF NOT EXISTS idx_funil_config_slug ON public.funil_config(slug);
CREATE INDEX IF NOT EXISTS idx_funil_config_terapeuta ON public.funil_config(terapeuta_id);
CREATE INDEX IF NOT EXISTS idx_funil_leads_terapeuta ON public.funil_leads(terapeuta_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_funil_leads_status ON public.funil_leads(status);
