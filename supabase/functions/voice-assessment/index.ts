...
const SYSTEM_PROMPT = `Você é um sistema de IA clínica especializado em reabilitação musculoesquelética, baseado em evidências científicas (Magee, Jull, Cook, Hodges, O'Sullivan, Butler & Moseley).
  ...
  function: {
    ...
    parameters: {
      ...
      properties: {
        transcricao: {
          type: "string",
          description: "Transcrição fiel do áudio em português (se recebeu áudio) ou a transcrição original (se recebeu texto)"
        },
        resumo_clinico: {
          type: "string",
...
serve(async (req) => {
  ...
  try {
    ...
    const serviceLabel = serviceType === 'identidade' ? 'Método Identidade (MyID)' :
    ...
    // Separate transcription from assessment data
